...
> 当前发布包尚未进行 Apple Developer ID 签名与公证，Windows 安装包也尚未签名。首次启动时可能显示来自未知开发者的安全提示，请只使用本仓库 Releases 页面提供的安装包。

### macOS 提示应用已损坏或无法验证

将 ProtoHub 拖入“应用程序”目录后，在终端执行：

```bash
xattr -dr com.apple.quarantine "/Applications/ProtoHub.app"
open "/Applications/ProtoHub.app"
```

`xattr` 命令会移除本机为此次下载添加的隔离标记，它不等同于开发者签名。请仅在确认安装包来自 [ProtoHub Releases](https://github.com/xnzone/protohub/releases/latest) 时使用。
...
ProtoHub 使用 [MIT License](LICENSE) 开源。
